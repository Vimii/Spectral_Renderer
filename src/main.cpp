...
spectrum ray_spectrum(const ray& r, const spectrum& background, const hittable& world,shared_ptr<hittable> lights, int depth) {
    ...
    if(srec.mat_type == diel && !r.separated){
        ray scattered[SAMPLE_NUM];
        spectrum ray_dispersion[SAMPLE_NUM];
        double pdf_val[SAMPLE_NUM] = {0};
        int sample_start = 400;
        int sample_end = 700;
        for (int j = getIndex(sample_start); j <= getIndex(sample_end); ++j) {
            scattered[j] = ray(rec.p,p.generate(),r.time(), getWaveLength(j),true);
            pdf_val[j] = p.value(scattered[j].direction());
            ray_dispersion[j] = ray_spectrum(scattered[j],background,world,lights,depth-1);
        }
        spectrum ray_sum = spectrum();
        for (int j = 0; j < SAMPLE_NUM; ++j) {
            if(j < getIndex(sample_start) || j> getIndex(sample_end)){
                ray_sum[j] = 0;
            }else {
                ray_sum[j] = emitted[getIndex(scattered[j].wavelength())]
                             + srec.attenuation[j] * rec.mat_ptr->scattering_pdf(r, rec, scattered[j])
                               * ray_spectrum(scattered[j], background, world, lights, depth - 1)[j] / pdf_val[j];
            }
        }
        return ray_sum;
    }
...
//        if (!objectId) return -1;
        ...
        for (int j = image_height - 1; j >= 0; --j) {
            ...
            for (int i = 0; i < image_width; ++i) {
                ...
                spectrum pixel_spectrum = spectrum();
                #pragma omp parallel for
